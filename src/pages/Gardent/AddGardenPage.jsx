...
import {
  SaveGarden_UploadMutli,
  SeachGarden,
  UpdateGarden_UploadMutli,
...
import AddGardenDetailComponent from "./AddGardenDetailComponent";
import AddPurchaseBillDetailComponent from "../PurchaseBill/AddPurchaseBillDetailComponent";
...
const AddGardenPage = () => {
  ...
  const [gardenDetail, setGardenDetail] = React.useState([]);
  ...
  const SaveData = async () => {
    ...
    if (TYPE_OF_DATA_IMG_RETURN === "file") {
      const formData = jsonToFormData({
        ...request,
        gardenDetail: JSON.stringify(request.gardenDetail),
        numberOfPlants: sumTotal,
      });
    ...
    SaveGarden_UploadMutli(request_v2)
      .then((response) => {
        if (response.success) {
  ...
  const UpdateData = async () => {
    ...
    UpdateGarden_UploadMutli(id, request_v2)
      .then((response) => {
        if (response.success) {
          navigation("/gardens");
  ...
  const LoadData = async () => {
    SeachGarden(id).then((response) => {
      if (response.success) {
        setRequest(response.data);
        setImages(response.data.images);
        setGardenDetail(response.data.gardenDetail);
      }
    });
  };
  ...
  console.log("req", sumTotal);

  return (
    <div>
      <PageBreadcrumb
        pageTitle={isEdit ? "Edit" : "Add new"}
        prePageTitle="Gardens"
        preLink="gardens"
      />
      <div className=" min-h-[calc(100vh-180px)] custom-scrollbar overflow-hidden  rounded-2xl border border-gray-200 bg-white px-4 pb-3 pt-4 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div>
            ...
            <Input type="number" readOnly value={sumTotal} />
            {/* <InputNumber
              value={request?.numberOfPlants ? request?.numberOfPlants : ""}
              onChange={(e) => {
                setRequest({ ...request, numberOfPlants: e });
              }}
            /> */}
          </div>
          <div>
            ...
            />
            {/* <InputNumber
              value={request?.landArea ? request?.landArea : ""}
              onChange={(e) => {
                setRequest({ ...request, landArea: e });
              }}
            /> */}
          </div>
          {/* <div className="col-span-1 md:col-span-4">
            <Label children={"Description"} />
            <AddGardenDetailComponent />
          </div> */}
          <div className="col-span-1 md:col-span-4">
            <Label children={`Detail plants`} />

            <AddGardenDetailComponent
              initData={request?.gardenDetail}
              onChange={(e) => {
                setRequest({
                  ...request,
                  gardenDetail: e,
