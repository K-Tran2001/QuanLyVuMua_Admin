...
const MyPagination = (props) => {
  const { filterPage, setFilterPage, totalRecords = 100 } = props;
  ...
  const handlePageSizeChange = (current, newSize) => {
    console.log("Số item mỗi trang:", newSize);
    setPageSize(newSize);
      ...
      <div className="flex items-center justify-between gap-2 px-6 py-4 sm:justify-normal">
        <Pagination
          ...
          itemRender={(page, type) => {
            if (type === "prev")
              return (
                <button className="dark:text-white cursor-pointer flex items-center justify-center  h-9 w-20  rounded-lg  hover:bg-brand-500 hover:text-white">
            ...
            return (
              <div
                className={`dark:text-white cursor-pointer flex h-9 w-9 items-center justify-center rounded-lg  hover:bg-brand-500 hover:text-white ${
                  page === currentPage
