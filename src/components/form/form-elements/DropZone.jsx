...
const DropzoneComponent = ({
  ...
  const handleImageUpload = (file) => {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        setData({ fileName: file.name, identityImage: e.target.result });
        // localStorage.setItem(
        //   "base64Img",
        //   e.target.result.replace("data:image/png;base64,", "")
        // );
        onUpload(typeDataReturn === "file" ? file : e.target.result);
      };
  ...
  return (
    <div>
      {(multiple || (!multiple && data.identityImage === "")) && (
        <div
      ...
      {data.identityImage != "" && (
        <div className="flex items-center space-x-4 border border-gray-300 dark:border-gray-700 rounded-lg relative my-4 p-2">
          <div className=" w-[100px] h-[100px]  ">
            ...
            />
            <div
              className="absolute top-0 right-0  translate-x-2 -translate-y-2 p-2 bg-gray-800 text-white rounded-lg dark:bg-white dark:text-black"
              onClick={() => setData({ fileName: "", identityImage: "" })}
            >
              <CloseIcon className="size-5 " />
            </div>
          </div>
          <h3 className="text-lg  flex-1 truncate ">{data.fileName}</h3>
        </div>
      )}
    </div>
  );
